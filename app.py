# -*- coding: utf-8 -*-
# 약국 찾기: 주소검색 + 지도클릭 + 영업중필터 + 결과유지
# KeyError/누락 대응 + Overpass 태그 확장(amenity|healthcare|shop)
...
# --- 세션 기본값 ---
for k, v in [
    ("last_df", None), ("last_center", None),
    ("last_radius", 1200), ("pending_center", None)
]:
...
def tz_at(lat, lon):
    try:
        ...
        if tzname: return pytz.timezone(tzname)
    except Exception:
        pass
    ...
    for i in range(tries):
        ...
        try:
            ...
            if code in (429,502,503,504):
                err = he; _time.sleep(backoff**i); continue
            raise
        except Exception as e:
            err = e; _time.sleep(backoff**i); continue
...
def _rule(s):
    m = re.match(r'^([A-Za-z]{2}(?:-[A-Za-z]{2})?(?:,\s*[A-Za-z]{2}(?:-[A-Za-z]{2})?)*)\s+([\d:]{4,5}-[\d:]{4,5}(?:\s*,\s*[\d:]{4,5}-[\d:]{4,5})*)$', s.strip())
...
with st.form("addr"):
    ...
    with c1:
        q = st.text_input("주소/장소명 (예: 남양주시청, 강남역)", "")
    with c2:
        gogo = st.form_submit_button("주소로 위치 지정")
if gogo and q.strip():
    try:
...
if submit:
    ...
    # amenity=pharmacy OR healthcare=pharmacy OR shop=chemist
    query = f"""
    [out:json][timeout:25];
    (
    ...
    try:
        ...
        for el in elems:
            ...
            o_now, o_disp = is_open_now(opening, now_local)
            dist = round(haversine((lat,lon),(plat,plon), unit="m"))
            rows.append({
                "이름": name, "거리(m)": dist,
                "영업여부": "영업중" if o_now is True else ("영업종료" if o_now is False else "확인필요"),
                "영업시간": o_disp, "전화": phone,
                "위도": plat, "경도": plon,
        ...
        # ✅ 빈 결과여도 컬럼 유지
        df = pd.DataFrame(rows, columns=["이름","거리(m)","영업여부","영업시간","전화","위도","경도","네이버지도","카카오맵"])
